...
public class Logger
{
	...
	public static synchronized Logger getLogger(String name, int level)
	{
		...
		if (log.getLevel() != level) log.setLevel(level);
	...
	{
		if (this.level > LEVEL_DEBUG) return;
	...
	{
		if (this.level > LEVEL_WARN) return;
	...
	{
		if (this.level > LEVEL_ERROR) return;
	...
	{
		if (this.level > LEVEL_FATAL) return;
