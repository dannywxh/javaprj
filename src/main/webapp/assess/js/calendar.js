...
//		//$("#detail").hide();
//		$("#detail").show();
//		var left=$(this).position().left + $(this).parent().width()*2+20;
//		var top=$(this).position().top + $(this).parent().height()-20;
//		detailDiv.css("margin-left",left);
//		detailDiv.css("margin-top",top);
//		detailDiv.html("1.省级警综平台的开发<br/>" +
//				       "2.工作平台的开发");
//	});
//	$("table tr td a").mouseout(function(){
//		//$("#detail").show();
//		$("#detail").hide();
//	});
		...
		success:function(json){
			if(json.length>0){
				...
				for(var i=0;i<json.length;i++){
					jjrmodelidlist.push(json[i].id);
					jjrmodeltimelist.push(json[i].sj);
					jjrmodelztlist.push(json[i].lx);
				}
...
//	szrq.appendChild(szrqbutton);
	...
	var rwallDiv=$(".rwallshow");
	var time=year+"-"+yue+"-"+day;
	var html="";
	//获取当天开始的任务
	$.ajax({
		type:"POST",
		url:"calendar/getCurrentDayRw",
		dataType: "json",
		async:false,
		data:{"time":time},
		success:function(json){
			html+="该天共<font color='#ff33cc'>"+(json.length)+"</font>项开始的任务<br/>"
			for(var i=0;i<json.length;i++){
				html+=(i+1)+".<a target='_blank' href='rw/showRw?id="+json[i].id+"'>"+json[i].bt+"</a><br/>";
			}
			rwallDiv.html(html);
		}
	...
	$.ajax({
		type:"POST",
		url:"calendar/getCurrentDayJsRw",
		dataType: "json",
		async:false,
		data:{"time":time},
		success:function(json){
			html+="该天共<font color='#ff33cc'>"+(json.length)+"</font>项结束的任务<br/>"
			for(var i=0;i<json.length;i++){
				html+=(i+1)+".<a target='_blank'  href='rw/showRw?id="+json[i].id+"'>"+json[i].bt+"</a><br/>";
			}
			rwallDiv.html(html);
...
function setA(){
	...
	//根据当前页面显示的日历信息查询该时间段后的所有任务
	var rws=[];
	$.ajax({
		type:"POST",
		url:"calendar/getRwByStartTime",
		dataType: "json",
		async:false,
		data:{"start":arr[0].getAttribute("date"),"end":arr[arr.length-1].getAttribute("date")},
		success:function(json){
			if(json.length>0){
				rws=json;
			}
	...
	for(var i=0;i<arr.length;i++){
		...
		var dateYue=date.substring(5,date.lastIndexOf("-"));
		var dateRi=date.substring(date.lastIndexOf("-")+1);
		dateYue=dateYue.length<2?"0"+dateYue:dateYue;
		dateRi=dateRi.length<2?"0"+dateRi:dateRi;
		var rwCountKs=0;
		var rwbtKs="";
		var flagKs=true;
		var rwCountJs=0;
		...
		var flagJs=true;
    ////////////////////////////开始任务遍历start/////////////////////////////////////
		for(var j=0;j<rws.length&&flagKs;j++){
			var rwKssjYear=rws[j].kssj.substring(0,4);
			var rwKssjYue=rws[j].kssj.substring(5,rws[j].kssj.lastIndexOf("-"));
			var rwKssjRi=rws[j].kssj.substring(rws[j].kssj.lastIndexOf("-")+1);
			
//			var rwJssjYear=rws[j].jssj.substring(0,4);
//			var rwJssjjYue=rws[j].jssj.substring(5,rws[j].jssj.lastIndexOf("-"));
//			var rwJssjRi=rws[j].jssj.substring(rws[j].jssj.lastIndexOf("-")+1);
			
			if((dateYear==rwKssjYear&&dateYue==rwKssjYue&&dateRi==rwKssjRi)
					//||dateYear==rwJssjYear&&dateYue==rwJssjjYue&&dateRi==rwJssjRi
					){
				rwCountKs++;
				if(rwCountKs<3){
					var btstr=rws[j].bt;
					if(btstr.length>6){
						btstr=btstr.substring(0,7)+"...";
					}
					rwbtKs+=btstr+"<br/>";
				}else{
					//超过这么多实在显示不下了……
		...
		var span_tsxx_ks=newElement('span');
		span_tsxx_ks.setAttribute("class","span_tsxx_ks");
		span_tsxx_ks.innerHTML=rwbtKs;
		arr[i].appendChild(span_tsxx_ks);
	/////////////////////////////开始任务遍历/////////////////////////////////////	
		
   //////////////////////////////结束任务遍历start///////////////////////////////
		for(var k=0;k<rws.length&&flagJs;k++){
			if(rws[k].jssj!=undefined){
			var rwJssjYear=rws[k].jssj.substring(0,4);
			var rwJssjjYue=rws[k].jssj.substring(5,rws[k].jssj.lastIndexOf("-"));
			var rwJssjRi=rws[k].jssj.substring(rws[k].jssj.lastIndexOf("-")+1);
			if(dateYear==rwJssjYear&&dateYue==rwJssjjYue&&dateRi==rwJssjRi){
				rwCountJs++;
				if(rwCountJs<2){
					var btstr=rws[k].bt;
					if(btstr.length>6){
		...
		var span_tsxx_js=newElement('span');
		span_tsxx_js.setAttribute("class","span_tsxx_js");
		span_tsxx_js.innerHTML=rwbtJs;
		arr[i].appendChild(span_tsxx_js);
   //////////////////////////////结束任务遍历start///////////////////////////////
