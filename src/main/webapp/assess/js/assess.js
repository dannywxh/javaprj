...
function initQuery(s,v,kpyf,xm){
	...
	if(v=="gcs"){
		$('#grid_index').skynetGrid({
    		...
    		columns: [
            	...
            	{title: '评分', field: 'pf', width: '100px',rowAlign: 'center',
            		formatter : function(val, row, index) {
            			return "<input onkeydown='return false;'  type='number' step = '1' max='5' min='-5' value="+val+" style='width:50px; margin-top:10px;' id='row_pf_"+row.id+"'><font color='red'>%</font>";
        			}	
            	},
            	{title: '备注', field: 'bz', width: '218px',rowAlign: 'center',
            		formatter : function(val, row, index) {
            			return "<textarea id='row_bz_"+row.id+"'>"+val+"</textarea>";
        			}	
            	},
            	{title: '总分', field: 'zf', width: '30px',rowAlign: 'center',
            	 ...
            	 {field: 'caozuo', title: '操作', isShowMore:false, width: '40px',rowAlign: 'center',
  	        	  formatter: function (val, row, index) {
  	        		var btn="<a  class='green icon-save' onclick='grading(\""+row.id+"\",\"gcs\")'>评分</a>";
       				return $(btn);
  	        	 }
  	          }
    	...
    	$('#grid_index').skynetGrid({
            ...
            isCheck: false,
    		columns: [
            	{title: '用户', field: 'xm', width: '25px',rowAlign: 'center',
            		formatter : function(val, row, index) {
        				return val;
        			}	
            	},
            	...
            	{title: '个人时间量', field: 'grsjl', width: '60px',rowAlign: 'center',
            		formatter : function(val, row, index) {
        				return val;
        			}	
            	},
            	{title: '提出人时间量', field: 'tcrsjl', width: '70px',rowAlign: 'center',
            		formatter : function(val, row, index) {
        				return val;
        			}	
            	},
            	{title: '时间评估量1', field: 'sjpgl1', width: '60px',rowAlign: 'center',
            		formatter : function(val, row, index) {
        				return val;
        			}	
            	},
            	{title: '时间评估量2', field: 'sjpgl2', width: '60px',rowAlign: 'center',
            		formatter : function(val, row, index) {
        				return val;
        			}	
            	},
            	{title: '时间评估量3', field: 'sjpgl3', width: '60px',rowAlign: 'center',
            		formatter : function(val, row, index) {
        				return val;
        			}	
            	},
            	...
            	{title: '评分', field: 'pf', width: '40px',rowAlign: 'center',
            		formatter : function(val, row, index) {
            			return "<input onkeydown='return false;'  type='number' step = '1' max='5' min='-5' value="+val+" style='width:30px; margin-top:10px;' id='row_pf_"+row.id+"'><font color='red'>%</font>";
        			}	
            	},
            	{title: '备注', field: 'bz', width: '100px',rowAlign: 'center',
            		formatter : function(val, row, index) {
            			return "<textarea id='row_bz_"+row.id+"'>"+val+"</textarea>";
        			}	
            	},
            	{title: '总分', field: 'zf', width: '20px',rowAlign: 'center',
            		formatter : function(val, row, index) {
            			var html="";
            			if(val<=0||val==null||val==""){
            				html="未评分";
            	 ...
            	 {field: 'caozuo', title: '操作', isShowMore:false, width: '20px',rowAlign: 'center',
  	        	  formatter: function (val, row, index) {
  	        		var btn="<a  class='green icon-save' onclick='grading(\""+row.id+"\",\"xmfzr\")'>评分</a>";
       				return $(btn);
  	        	 }
  	          }
		...
		$('#grid_index').skynetGrid({
    		...
    		columns: [
            	...
            	{title: '评分', field: 'pf', width: '50px',rowAlign: 'center',
            		formatter : function(val, row, index) {
            			return "<input  type='number' step = '10' min='0' value="+val+" style='width:50px; margin-top:10px;' id='row_pf_"+row.id+"' ><font color='red'></font>";
        			}	
            	},
            	{title: '备注', field: 'bz', width: '218px',rowAlign: 'center',
            		formatter : function(val, row, index) {
            	...
            	{title: '总分', field: 'zf', width: '50px',rowAlign: 'center',
            	 ...
            	 {field: 'caozuo', title: '操作', isShowMore:false, width: '40px',rowAlign: 'center',
  	        	  formatter: function (val, row, index) {
  	        		var btn="<a  class='green icon-save' onclick='grading(\""+row.id+"\",\"bmjl\")'>评分</a>";
       				return $(btn);
  	        	 }
  	          }
            ]
